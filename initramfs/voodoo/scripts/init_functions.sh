...
in_recovery()
{
	if test "`cut -d' ' -f 2 /proc/cmdline`" = "bootmode=2"; then
		log_suffix='-recovery'
		return 0
	else
		return 1
...
convert()
{
	...
	elif test "$model" = 'P1_gingerbread_16' || test "$model" = 'P1_gingerbread_32'; then
		battery_level=`cat /sys/devices/platform/sec_battery/power_supply/battery/capacity`
	else
...
letsgo()
{
	...
	# deal with the boot animation - doesn't work on Galaxy Tab -AB86
	#mount_ data
	#readd_boot_animation
	#umount /data
